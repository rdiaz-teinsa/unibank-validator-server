...
/*
function exportExcelToTxt(inputPath: string, outputPath: string, delimiter: string) {
    try {

        if (!fs.existsSync(inputPath)) {
            throw "El archivo especificado no existe.";
        }

        const ext = path.extname(inputPath).toLowerCase();
        if (ext !== ".xls" && ext !== ".xlsx") {
            throw "Formato no soportado. Solo se permiten archivos .xls o .xlsx.";
        }
        ...
        if (!sheet) {
            throw "No se encontró ninguna hoja en el archivo Excel.";
        }

        const jsonData: any[] = XLSX.utils.sheet_to_json(sheet, {defval: ""});

        if (jsonData.length === 0) {
            throw "La hoja de Excel está vacía.";
        }
...
exportExcelToTxt("/Users/arielherrera/WebstormProjects/unibank-validator-server/archive/data/236/20251014/test.xlsx", "/Users/arielherrera/WebstormProjects/unibank-validator-server/archive/data/236/20251014/test.txt", "~");
